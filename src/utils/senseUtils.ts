...
export const generateColorScale = (nodes: SenseNode[]): Map<string, string> => {
  ...
  return new Map(
    nodes.map((node, index) => [node.id, colorScale(`${node.parentId ? 1 : 0}${index % 10}`)]),
  )
}
 ...
 */
export const buildSenseNetwork = (
  senses: any[],
  word: string,
  lang: string,
  ...
  const centralNode: SenseNode = createNode(
    `central_${word}`,
    `${word} (${lang})`,
    [],
    true,
...
const processSubSenses = (
  ...
  sense.senseid.slice(1).forEach((subId: string) => {
    ...
    const position = calculatePosition(
      siblingCount,
      subSenseTracker.get(parentNode.id)!,
      parentNode.x!,
      parentNode.y!,
      subSpacingFactor,
    )

    const subNode = createNode(subId, subGloss, sense.tags || [], false, parentNode.id, position)
