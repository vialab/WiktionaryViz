...
export default function NetworkPage({ word1, word2, language1, language2 }: NetworkPageProps) {
  ...
  return (
    <div
      ref={wrapperRef}
      className="w-screen h-screen flex flex-col items-center justify-center bg-black overflow-hidden relative"
    >
...
function buildEtymologyGraph(
  ...
  derivations.forEach(template => {
    ...
    const nodeExists =
      existingNodes.find(node => node.id === nodeId) || newNodes.find(node => node.id === nodeId)
