    ...
    async function fetchTimelineFromAncestryChain() {
      ...
      try {
        const res = await fetch(
          apiUrl(
            `/ancestry-chain?word=${encodeURIComponent(word)}&lang_code=${encodeURIComponent(language)}`,
          ),
        )
        ...
        if (result.ancestry_chain && result.ancestry_chain.length > 0) {
          ...
          if (
            rootNode &&
            Array.isArray(rootNode.etymology_templates) &&
            rootNode.etymology_templates.length > 0 &&
            rootNode.etymology_templates[0].expansion
          ) {
            rootExpansion = rootNode.etymology_templates[0].expansion
          }
        }
        if (result.ancestry_chain && Array.isArray(result.ancestry_chain)) {
          const timeline: NodeData[] = result.ancestry_chain.map(
            (entry: AncestryChainEntry & { drift?: number }) => {
              ...
              if (!lang && result.ancestry_chain.length > 0) {
                ...
                if (rootNode && Array.isArray(rootNode.etymology_templates)) {
                  ...
                  if (match) {
                    ...
                    lang = match.expansion
                      .replace(new RegExp(`\\s*${wordToRemove}(\\s*(.*))?$`), '')
                      .trim()
                  }
                }
              }
              ...
              console.debug(
                `Processing entry: ${entry.word} (${lang}) - IPA: ${entry.ipa}, Phonemic: ${hasPhonemic}, AI Estimated: ${isAIPhonetic}, Data Quality: ${dataQuality}`,
              )
              return {
                lang,
                drift: entry.drift ?? 0,
