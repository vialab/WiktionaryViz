  ...
  if (drift && drift.alignment && Array.isArray(drift.alignment)) {
    driftInfo = (
      <div className="mt-2 text-xs text-[#D4AF37]">
        <span className="font-semibold">Phonetic Drift:</span><br />
        <span className="text-[#B79F58]">{drift.ipa1} → {drift.ipa2}</span><br />
        <ul className="mt-1 ml-2 list-disc">
          {drift.alignment.map((diff: PhoneticDriftChange, i: number) => (
            <li key={i} className="mb-1">
              {diff.from && diff.to ? (
                <>
                  <span className="font-mono">{diff.from}</span> → <span className="font-mono">{diff.to}</span>
                  {diff.changes && Object.keys(diff.changes).length > 0 && (
                    <span className="ml-2 text-[#B79F58]">[
                      {Object.entries(diff.changes ?? {}).map(([feat, val], j: number) => (
                        <React.Fragment key={j}>{feat}: {String(val)}{j < Object.entries(diff.changes ?? {}).length - 1 ? ', ' : ''}</React.Fragment>
                      ))}
                    ]</span>
                  )}
                </>
              ) : diff.status === 'deletion' ? (
                <span className="text-red-400">Deleted <span className="font-mono">{diff.from}</span></span>
              ) : diff.status === 'insertion' ? (
                <span className="text-green-400">Inserted <span className="font-mono">{diff.to}</span></span>
              ) : (
    ...
    driftInfo = (
      <div className="mt-2 text-xs text-[#D4AF37]">
        <span className="font-semibold">Phonetic Drift:</span><br />
        <span className="text-[#B79F58]">{prevCard.word} IPA:</span> <span className="font-mono">{prevCard.pronunciation}</span><br />
        <span className="text-[#B79F58]">{card.word} IPA:</span> <span className="font-mono">{card.pronunciation}</span>
      </div>
    );
  }
